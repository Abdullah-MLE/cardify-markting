...
class GeminiWrapper:
    ...
    def __init__(self, client: Optional[genai.Client] = None):
        ...
        Args:
            client: Optional Gemini client. If None, creates a new one using init_gemini_client().
        """
        self.client = client or init_gemini_client()
    ...
    def _download_media(self, url: str) -> tuple[bytes, str]:
        """Download media from URL or read local file and return (bytes, mime_type)."""
        try:
...
def init_gemini_client() -> genai.Client:
    """Initialize and return a Gemini client with proper GCP Vertex AI settings if enabled."""
    use_vertex = os.environ.get("GOOGLE_GENAI_USE_VERTEXAI", "True").lower() in ("true", "1")
    project = os.environ.get("GCP_PROJECT_ID", "project-d3cc105c-93d6-42fa-abc")
