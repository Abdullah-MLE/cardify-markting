...
def carousel_cover_user_prompt(content: dict, user_instructions: str = None) -> str:
...
def carousel_continuation_user_prompt(content: dict, slide_index: int) -> str:
    headline = content.get("h1", [])[slide_index] if content.get("h1", []) and slide_index < len(content.get("h1", [])) else ""
    post_idea = content.get("post_idea", "")[slide_index] if content.get("post_idea", "") and slide_index < len(content.get("post_idea", "")) else ""
    prompt = [
