...
def extract_company_system_prompt() -> str:
    prompt = [
        ...
        "company_name, industry, description, mission_and_goal, brand_tone, target_audience,",
        "language_and_locale, constraints, is_character, main_character_name, main_character_constraints,",
        "main_character_image_url, visual_constraints, visual_style, brand_color, logo_url,",
        "website_url, social_media_username, facebook_url, x_url, instagram_url, linkedin_url, tiktok_url.",
...
def edit_company_user_prompt(company_json: str, notes: str) -> str:
    prompt = [
        f"Current Details: {company_json}",
        f"User Notes: {notes}",
    ]
    return "\n".join(prompt)
